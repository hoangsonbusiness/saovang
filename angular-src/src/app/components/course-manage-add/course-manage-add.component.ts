...
@Component({
  selector: 'app-course-manage-add',
  templateUrl: './course-manage-add.component.html',
  styleUrls: ['./course-manage-add.component.css']
})
export class CourseManageAddComponent implements OnInit {
  fileNameArr = [];
  editor1:String = "";
  editor2:String = "";
  thumpbnail = "";
  ...
  addData(title){
    this.editor1 = this.editor1.replace("</html>","")
     ...
     this.editor2 = this.editor2.replace("</html>","")
                    ...
                    .replace("<head>","")
                    .replace("<body>","").trim();
    this.service.add(title, this.editor1, this.editor2, this.thumpbnail, this.fileNameArr)
            .subscribe(res => {
              alert('Đăng khóa học mới thành công !');
